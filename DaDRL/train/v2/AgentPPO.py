...
from DaDRL.train.net import ActorPPO, ActorDiscretePPO, CriticPPO, SharePPO
from DaDRL.train.AgentBase import AgentBase
...
class AgentPPO(AgentBase):
    ...
    def explore_one_env(self, env, target_step, random_exploration=None):
        """
        ...
        rewards = list()
        raw_rewards = list()
        get_action = self.act.get_di_action
        ...
        while step_i < target_step or not done:
            ...
            if done:
                rewards.append(acu_reward)
                acu_reward = 0

        self.states[0] = state
        last_done[0] = step_i
        return self.convert_trajectory(traj_list, last_done), self.convert_trajectory(dad_train_traj_list, last_done), rewards, raw_rewards, step_i  # traj_list
