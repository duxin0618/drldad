...
class ActorPPO(nn.Module):
    ...
    def get_di_action(self, state: Tensor) -> (Tensor, Tensor):
        a = self.net(state)
        return a.tanh(), torch.randn_like(self.net(state))
...
class ActorDiscretePPO(nn.Module):
    ...
    def get_di_action(self, state):
        a_prob = self.soft_max(self.net(state))
        samples_2d = torch.argmax(a_prob)
        # action = Categorical(a_prob).sample()
        # samples_2d = torch.multinomial(a_prob, num_samples=1, replacement=True)
        action = samples_2d.reshape(state.size(0))
        return action, a_prob
