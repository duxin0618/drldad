...
def train_and_evaluate(args, threshold):
    ...
    while if_train:
        while True:
            ii_len = ii_len + 1
            trajectory, dad_train_trajectory, explore_reward ,raw_rewards, steps_i = agent.explore_env(env, target_step)
        ...
        if useDaD:
            ...
            if useDaDTrain:
                ...
                # for i in range(2):
                    ...
                    dad_trajectory, dad_step, mean_model_explore_reward = dad.explore_env(env, target_step, dad_train_buffer, agent.act, rew_info)
                    print("cur_steps is: ", dad_step)
                    if dad_trajectory is False:
                        continue
                    if dad_step < 128:
                        continue
                    steps, r_exp = buffer.update_buffer((dad_trajectory,))
                    train_number += 1
                    use_dad_trains += 1
                    dad_steps += dad_step
                    ...
                    print("use dad train numbers is :", use_dad_trains, " cur_steps is ", dad_step, "sum_dad_explore_steps: ", dad_steps)
                    ...
                    if steps // (control_out_num) > 0 :
                        control_out_num = control_out_num + 1e4
                        if useDaD:
