...
class Evaluator:  # [ElegantRL.2022.01.01]
    def __init__(self, cwd, agent_id, eval_env, args):
        ...
        self.eval_times1 = int(args.eval_times)
    ...
    def save_explorerewards_curve_plot_and_npy(self, cwd, rewards, useDad, threshold=None):
        explorerewards_curve_plot(cwd, rewards, useDad, threshold=threshold)
...
def explorerewards_curve_plot(cwd, rewards : list, useDad=False, threshold=None):
    ...
    if useDad is False:
        plt.plot(length, rewards, label="threshold=None", color=color)
    else:
        plt.plot(length, rewards, label="threshold="+str(threshold), color=color)
