...
class AgentBase:
    def __init__(self, net_dim: int, state_dim: int, action_dim: int, gpu_id=0, args=None):
        self.gamma = getattr(args, 'gamma', 0.99) # args 设置变量gamma为0.99
    ...
    def convert_traj_to_transition(self, traj_list):
        ...
        # 将 mask 作为 gamma 如果done=false，1-false = 1， 即mask = 0.97
...
class AgentPPO(AgentBase):
    def __init__(self, net_dim, state_dim, action_dim, gpu_id=0, args=None):
        ...
        self.act_class = getattr(self, "act_class", ActorPPO) # 给act_class赋初值为ActorPPO
        self.cri_class = getattr(self, "cri_class", CriticPPO)
        args.if_act_target = getattr(args, 'if_act_target', False) # 不使用act_target
    ...
    def explore_env(self, env, target_step: int): #  -> list
        traj_list = []
        state = self.states[0] # 随机初始状态
        sum_reward = 0
        done_num = 0
        i = 0
        done = False
        get_action = self.act.get_action  # 获取对象中的方法
        convert = self.act.convert_action_for_env
        while i < target_step or not done:   # target_step =  args.max_step * 8 = 200 * 8 = 1600
            # 转化为张量
            tensor_state = torch.as_tensor(state, dtype=torch.float32).unsqueeze(0)
            # 获取带有噪声的随机行为，与对应的噪声
            tensor_action, tensor_noise = [ten.cpu() for ten in get_action(tensor_state.to(self.device))]
            # get next state etc. 传入的动作需要扩张到对应的维度
        ...
        self.states[0] = state
        # 将信息变形，过渡
        return self.convert_traj_to_transition(traj_list), sum_reward / done_num
...
class ReplayBuffer:  # for off-policy
    ...
    def update_buffer(self, traj_list):
        ...
        return steps, r_exp, sum(rewards)
