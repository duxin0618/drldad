...
def train_agent(args):
    ...
    while True:
        trajectory, mean_rew = agent.explore_env(env, target_step)
        ...
        if eval_timer + eval_gap < time.time():
            ...
            print(f"| Steps {total_step:8.2e}  ExpR {r_exp:8.2f}  "
                    f"| trajectory_mean_reward {mean_rew:8.2f} "
...
def evaluate_agent(args):
    ...
    for model_name in model_names:
        for _ in range(50):
            ...
            rewards_steps_ary = [get_rewards_and_steps(env, actor) for _ in range(args.eval_times)] # 8 * 10
...
def get_rewards_and_steps(env, act) -> (float, int):  # get cumulative_rewards and episode steps
    ...
    for episode_steps in range(env.max_step): # 50
        ...
        if done:
            break
    # 如果更换不同的环境，适当调节 for 循环的遍历次数
    # 轨迹保存在 trajectory/(环境名里)

    add_file(envName)
    open_file_and_save('trajectory/'+ envName +'/observations.csv', observations)
    open_file_and_save('trajectory/'+ envName +'/actions.csv', actions)
    open_file_and_save('trajectory/'+ envName +'/next_observations.csv', next_observations)
...
def add_file(envName):
    import os.path
    if not os.path.exists('trajectory/'+envName):
        os.makedirs("trajectory/"+envName)